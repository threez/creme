 ...
 * don't leak to its parent) without icecreme's flat slot-table compilation
 ...
 * RecordCallable's own doc comment in value.h for why: icecreme's plain
 ...
 * produced this chunk: native's icecreme_emitter.cr serializes a real
 ...
 * primitives icecreme has a fused fast path for" (or an arity that doesn't
...
static Value creme_dispatch(VM *vm, int target_depth) {
  ...
  static const void *dispatch_table[OP_QUICK_COUNT] = {
       ...
       * implemented here — see icecreme/README.md for current coverage. Each
...
#define NEXT() break
  ...
  CASE(OP_HELPERFORM) {
     ...
     * icecreme/README.md), so a no-op here is correct. c=1 (define-library)
     ...
     * icecreme and later compiles a USE of it — matching what Crystal's own
 ...
 * icecreme/mux.c's request handler. A T_BUILTIN just runs directly (no frame
