# icecreme stability policy

This project (both `icecreme` and the native `creme` interpreter it shares a
...
precompiled `.ice`) knows what needs re-checking after an upgrade and
what doesn't.
...
- **The ICE1 bytecode format's read compatibility, gated by an explicit
  ...
  effect of ordinary feature work. A `.ice` compiled at one point
...
- **The base builtin procedure set** — R7RS `(scheme base)` and the
  other standard libraries `icecreme/README.md`'s "Compatibility with creme"
...
- **Everything in `vm.h`/`value.h`/`opcodes.h` not listed above** —
  ...
  ABI yet: linking against icecreme's internals means recompiling against
...
- **`(creme actor)`'s wire protocol** (the TCP/Unix transport's own
  ...
  treat cross-version actor clusters (an old icecreme build talking to a
