 ...
 * icecreme/README.md for the overall design and what's deliberately NOT
 * implemented. */
#ifndef CREME_VM_H
#define CREME_VM_H
...
/* HOT struct: exactly what creme_dispatch's NEXT() fetches, once per executed
 ...
 * doc/optimization-icecreme.md's Section 1. */
typedef struct {
  int op, a, b, c, d;
} Instruction;
 ...
 * the real format's debug info; icecreme doesn't currently use it for anything
 ...
 * QQVector) — see creme_serializer.cr's write_qq_template for the on-disk
 ...
 * raised arbitrarily deep (including through creme_apply's own reentrant C
 ...
 * never reallocated after creme_alloc_vm builds it (see its own doc
 * comment, and CREME_DEFAULT_FRAMES_CAP's, for why). */
typedef struct {
  jmp_buf buf;
  int depth;
 ...
 * directly in C -- NOT via creme_apply'ing a Scheme thunk the way
 ...
 * lifetime, allocated once (by creme_alloc_vm, vm.c) and never reallocated
 ...
 *
 * CREME_DEFAULT_STACK_CAP/CREME_DEFAULT_FRAMES_CAP are just that -- the
 * defaults creme_alloc_vm falls back to when asked for 0, not a hard
 * limit baked into the VM struct's own layout the way they used to be
 * (`Value stack[ICECREME_STACK_CAP]`/`Frame frames[ICECREME_FRAMES_CAP]` as fixed
 * array MEMBERS). An embedder wanting to cap a given script's resource
 * use tighter (or looser) than these calls creme_alloc_vm with its own
 * values instead -- see that function's own doc comment. */
#define CREME_DEFAULT_STACK_CAP (1 << 20)
#define CREME_DEFAULT_FRAMES_CAP 8192
#define CREME_GLOBALS_CAP 4096
#define CREME_HANDLERS_CAP 256
#define CREME_UNWIND_CAP 1024
#define CREME_EXC_HANDLERS_CAP 256
...
#define CREME_PROFILE_VM_SAMPLES_CAP 4096
typedef struct {
  Chunk *chunk;
  int ip;
  long count;
...
/* Shared across a profiled VM AND every child VM creme_new_child_vm ever
 ...
 * enabled tracking on, so without this, creme_profiler_tick's samples
 ...
 * creme_profiler_tick's own countdown check, still lock-free and
 * per-VM-instance), so this adds no meaningful per-instruction cost. */
typedef struct {
  VmSample vm_samples[CREME_PROFILE_VM_SAMPLES_CAP];
  int n_vm_samples;
  pthread_mutex_t mu;
} SharedVmSamples;
...
#define CREME_PROFILE_NATIVE_SAMPLES_CAP 65536
typedef struct {
  void *pcs[16];
  int n_pcs;
} NativeSample;
...
typedef struct {
   ...
   * ticked from creme_dispatch's NEXT() — see profiler.c's creme_profiler_tick.
   ...
   * narrowly around the profiled run by creme_profiler_start_native/_stop_native. */
  struct sigaction old_sigprof_action;
  struct itimerval old_itimer;
  NativeSample *native_samples; /* GC_MALLOC'd array, CREME_PROFILE_NATIVE_SAMPLES_CAP entries */
...
struct VM {
  Value *stack;      /* GC_MALLOC'd once by creme_alloc_vm, sized stack_cap */
  int stack_cap;
  Frame *frames;     /* GC_MALLOC'd once by creme_alloc_vm, sized frames_cap */
  int frames_cap;
  int depth;
  GlobalCell globals[CREME_GLOBALS_CAP];
  int n_globals;
  GuardHandler handlers[CREME_HANDLERS_CAP];
  int n_handlers;
  UnwindAction unwind_stack[CREME_UNWIND_CAP];
  int n_unwind;
  ...
  int n_exc_handlers;
  Value pending_condition;   /* set by creme_raise_condition right before its
                               * longjmp; read back by GuardReraise. */
  int pending_handler_idx;  /* set by creme_raise_condition right before its
                              ...
                              * (no intervening creme_apply recursion): a
  ...
  RecordType *condition_type; /* one process-wide type shared by every
                               * condition creme_abort/error construct --
   ...
   * objects (builtins.c's creme_init_current_ports) -- one per VM
   * instance, which already gives them the same per-actor-thread
   * independence a `_Thread_local` C global would (see creme_new_child_vm,
   ...
   * default), so an uncaught creme_abort inside that actor's thunk longjmps
   * here (see creme_abort, vm.c) instead of exit()ing the whole process —
  ...
  /* creme_cons's (vm.c) own batch-refilled freelist -- GC_malloc_many
   ...
   * nothing is ever put back once creme_cons links it into a real pair. */
  void *pair_freelist;
};
 ...
 * (opening and closing the file itself, independently of creme_load) so a
 * caller -- main.c -- can decide which builtin families to register BEFORE
 * calling creme_load for real. See loader.c's own doc comment on this
 * function for why this is a separate open rather than sharing a Reader
 * with creme_load. `names_out`/`count_out` follow the same GC_MALLOC'd-array/
 * NUL-terminated-C-string convention as creme_load's own out params; pass
 ...
 * afterward). Either argument may be 0 to take the CREME_DEFAULT_STACK_CAP/
 * CREME_DEFAULT_FRAMES_CAP default. Every VM-creation site in this codebase
 * (the main script's own top-level VM in main.c, creme_new_child_vm's actor
 * spawn, creme_new_empty_vm's environment/eval target, all below) goes
 ...
 * creme_port_read_char/creme_port_peek_char return -1 at EOF, else a byte
...
/* A 64-bit hash consistent with creme_equal (any a, b with creme_equal(a,b) must
 * have creme_hash_value(a) == creme_hash_value(b)) -- see builtins.c's own
 * comment next to creme_equal_rec for how it mirrors that function tag-for-
 ...
 * VM-struct doc comment above. Called once by creme_register_base_builtins
 * for the top-level VM, and again by creme_new_child_vm (vm.c) for every
 ...
 * the target file path (icecreme has no other flags a script consumer would
 ...
 * site throughout icecreme (there are hundreds, none of them NULL-checked --
 ...
 * same clean "icecreme: out of memory" abort every OTHER resource-exhaustion
 ...
 * existing creme_abort machinery, so a script running under an installed
 * `guard` gets a genuinely catchable condition for this too, not just a
 * hard process exit -- the same distinction creme_abort already makes for
