...
/* config.h first, per autoconf convention -- generated by ./configure from
 * configure.ac's AC_CHECK_FUNCS/etc, defines HAVE_CLOSEFROM below. */
#include "config.h"
...
/* HAVE_CLOSEFROM (config.h, from configure.ac's AC_CHECK_FUNCS) -- a real
 * function-existence check rather than a hardcoded BSD-platform-name list.
 * The latter used to wrongly assume __APPLE__ has it too (macOS's libc/SDK
 * headers never declare it), which only surfaced as a build failure on
 * macOS -- testing the actual feature means whichever platforms genuinely
 * have it get the fast path automatically, no per-platform-name guesswork
 * to keep in sync as new platforms show up. */
