; Prints each competition/bench/workloads.scm workload's result plus its
; wall-clock time via (scheme time)'s current-second (R7RS's minimal time
; contract), then a "total = ...s" line — the plain-text creme.scm and
; racket.scm output that competition/bench.scm's own parse-elapsed-alist
; regexp expects.
;
; A separate file from workloads.scm itself (included right after it, not
; folded in) so that competition/bench/prof.scm — which (include
; "workloads.scm") only to profile those same definitions — doesn't also pay
; for and print this demo run first; it has its own profiling report to
; produce instead.
...
(timed-run "hashtable-test(200000)" (lambda () (hashtable-test 200000)))
(timed-run "record-test(500000)" (lambda () (record-test 500000)))
