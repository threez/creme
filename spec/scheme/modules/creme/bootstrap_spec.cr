...
# Compiles `source` the same way Scheme.run_source would, serializes the
# resulting Chunk to "SCB1" bytes via ChunkSerializer, and hands those bytes
...
private def compiled_bytes(interp : Scheme::Interpreter, env : Scheme::Env, source : String) : Scheme::SchemeBlob
  forms = Scheme::Reader.read_all(source)
  node = interp.analyze(Scheme::Cons.new(Scheme::SchemeSym.of("begin"), Scheme.a_to_list(forms)), env)
  chunk = Scheme::BytecodeCompiler.compile_program([node])
  Scheme::SchemeBlob.new(Scheme::ChunkSerializer.serialize(chunk))
end
...
describe "bootstrap module" do
  it "runs a deserialized chunk that matches direct evaluation" do
    ...
    run_result = Scheme.run_source(interp, "(load-chunk-bytes chunk-bytes)")
  ...
  it "round-trips closures, recursion, and strings/vectors" do
    ...
    interp.global.define("chunk-bytes", blob)
    result = Scheme.run_source(interp, "(load-chunk-bytes chunk-bytes)")
    result.write_string.should eq(%("sum=124"))
  end

  it "raises a clean error on garbage bytes" do
    expect_raises(Scheme::SchemeRuntimeError, /load-chunk-bytes/) do
  ...
  it "import! copies a library's bindings into the global env" do
    ...
    Scheme.run_source(interp, %((regexp-matches? (regexp "a+") "aaa"))).write_string.should eq("#t")
  end
