...
private def w(src : String) : String
  ...
  Creme.run_source(interp, "(import (scheme base) (creme reader)) #{src}").write_string
end

private def run(src : String) : Creme::SchemeValue
  interp = Creme::Interpreter.new(library_search_path: ["./modules"])
  Creme.run_source(interp, "(import (scheme base) (creme reader)) #{src}")
end
...
describe "(creme reader)" do
  it "round-trips ordinary forms through lex-tokens + tokens->forms unchanged" do
    ["(f x y)", "(define (add x y) (+ x y))", "'(1 2 3)", "\"a string\" 42 3.5"].each do |src|
      expected = Creme::Reader.read_all(src, "t").map(&.write_string).join(" ")
  ...
  it "tokens->forms rejects a malformed token" do
    expect_raises(Creme::SchemeRuntimeError, /malformed token/) do
      run(%[(tokens->forms (list (list 'symbol "x")) "t")])
    end
  end

  it "tokens->forms rejects an unknown token kind" do
    expect_raises(Creme::SchemeRuntimeError, /unknown token kind/) do
