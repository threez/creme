...
describe Scheme::Lambda do
  ...
  it "defaults name to lambda" do
    Scheme::Lambda.new([] of String, nil, [] of Scheme::SchemeValue, Scheme::Env.new).name.should eq("lambda")
  end
  ...
  it "displays as #<procedure:name>" do
    lam = Scheme::Lambda.new([] of String, nil, [] of Scheme::SchemeValue, Scheme::Env.new, "adder")
...
describe Scheme::SchemeHashTable do
  ...
  it "index_of finds a key by equal?, not identity" do
    ...
    entries = [{key1, Scheme::SchemeInt.new(99_i64).as(Scheme::SchemeValue)}]
    h = Scheme::SchemeHashTable.new(entries)
...
describe Scheme::SchemePromise do
  it "displays as unforced before being forced" do
    env = Scheme::Env.new
    Scheme::SchemePromise.new(Scheme::NIL, env).display_string.should eq("#<promise>")
  end
