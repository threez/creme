...
# `Scheme::VM.new(interp, interp.global).run(chunk)` pattern for running a
# freestanding Chunk.
private def run_chunk(chunk : Scheme::Chunk) : Scheme::SchemeValue
...
# counter, reg 2 = limit.
private def counted_sum_chunk(start : Int32, limit : Int32, step : Int32) : Scheme::Chunk
  chunk = Scheme::Chunk.new
  chunk.num_registers = 3
  acc, counter, lim = 0, 1, 2
  chunk.emit(Scheme::Op::LoadK, acc, chunk.consts.size)
  chunk.consts << Scheme::SchemeInt.new(0_i64)
  chunk.emit(Scheme::Op::LoadK, counter, chunk.consts.size)
  chunk.consts << Scheme::SchemeInt.new(start.to_i64)
  chunk.emit(Scheme::Op::LoadK, lim, chunk.consts.size)
  chunk.consts << Scheme::SchemeInt.new(limit.to_i64)

  prep_ip = chunk.emit(Scheme::Op::ForPrep, counter, 0, lim, step)
  body_start = chunk.instructions.size
  chunk.emit(Scheme::Op::Add, acc, acc, counter)
  loop_ip = chunk.emit(Scheme::Op::ForLoop, counter, 0, lim, step)
  chunk.instructions[loop_ip] = Scheme::Instruction.new(
    Scheme::Op::ForLoop, counter, body_start - (loop_ip + 1), lim, step
  )
  chunk.patch_jump_to_here(prep_ip)
  chunk.emit(Scheme::Op::Return, acc)
  chunk
end
...
private def run_program(src : String) : Scheme::SchemeValue
  interp = Scheme::Interpreter.new(library_search_path: ["./modules"])
...
# disassembler.
private def compiled_body_ops(src : String) : Array(Scheme::Op)
  interp = Scheme::Interpreter.new(library_search_path: ["./modules"])
  Scheme.run_source(interp, "(import (scheme base))")
  forms = Scheme::Reader.read_all(src)
  chunk = Scheme::Chunk.new
  forms.each do |form|
...
describe "self-recursive global counted-loop fusion" do
  it "fuses a plain tail-recursive accumulator define (decrementing counter)" do
    ...
    compiled_body_ops("(define (sum-to n acc) (if (= n 0) acc (sum-to (- n 1) (+ acc n))))")
      .should contain(Scheme::Op::ForLoopGuardedDec)
  end
  ...
  it "fuses an incrementing counted self-recursive define" do
    ...
    compiled_body_ops("(define LIMIT 100000) (define (count-up i acc) (if (= i LIMIT) acc (count-up (+ i 1) (+ acc i))))")
      .should contain(Scheme::Op::ForLoopGuardedInc)
  end
  ...
  it "does NOT fuse a step other than +-1 (falls back to the ordinary TailCallGlobal path unchanged)" do
    ...
    ops.should_not contain(Scheme::Op::ForLoopGuardedDec)
    ops.should contain(Scheme::Op::TailCallGlobal)
