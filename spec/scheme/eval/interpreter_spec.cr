...
describe Creme::Interpreter do
  describe "quote" do
    ...
    it "raises for the wrong number of arguments" do
      expect_raises(Creme::SchemeRuntimeError, /quote: expects 1 argument/) do
        run("(quote a b)")
      end
    end
  end
  ...
  describe "quasiquote/unquote/unquote-splicing" do
    ...
    it "raises when unquote appears outside quasiquote" do
      expect_raises(Creme::SchemeRuntimeError, /unquote: not valid outside quasiquote/) do
        run("(unquote 1)")
      end
    end

    it "raises when unquote-splicing appears outside quasiquote" do
      expect_raises(Creme::SchemeRuntimeError, /unquote-splicing: not valid outside quasiquote/) do
  ...
  describe "if" do
    ...
    it "raises for the wrong arity" do
      expect_raises(Creme::SchemeRuntimeError, /if: expects 2 or 3 arguments/) do
        run("(if #t)")
      end
    end
  end
  ...
  describe "cond" do
    ...
    it "raises on an empty clause" do
      expect_raises(Creme::SchemeRuntimeError, /cond: empty clause/) do
        run("(cond ())")
      end
    end
  end
  ...
  describe "case" do
    ...
    it "raises on an empty clause" do
      expect_raises(Creme::SchemeRuntimeError, /case: empty clause/) do
        run("(case 1 ())")
      end
    end

    it "raises when the key expression is missing" do
      expect_raises(Creme::SchemeRuntimeError, /case: expects a key expression/) do
        run("(case)")
      end
    end
  end
  ...
  describe "when/unless" do
    ...
    it "raises when 'when' has no condition" do
      expect_raises(Creme::SchemeRuntimeError, /when: expects a condition/) do
        run("(when)")
      end
    end

    it "raises when 'unless' has no condition" do
      expect_raises(Creme::SchemeRuntimeError, /unless: expects a condition/) do
        run("(unless)")
      end
    end
  end
  ...
  describe "define" do
    ...
    it "names an anonymous lambda when bound via define" do
      interp = Creme::Interpreter.new(library_search_path: ["./modules"])
      Creme.run_source(interp, "(define f (lambda (x) x))")
  ...
  describe "set!" do
    ...
    it "raises for an unbound variable" do
      expect_raises(Creme::SchemeRuntimeError, /set!: unbound variable: y/) do
        run("(set! y 1)")
      end
    end

    it "raises when the target isn't a symbol" do
      expect_raises(Creme::SchemeRuntimeError, /set!: first argument must be a symbol/) do
        run("(set! 1 2)")
      end
    end
  end
  ...
  describe "lambda" do
    ...
    it "raises for too few args with a rest param" do
      expect_raises(Creme::SchemeRuntimeError, /expected at least 1 argument\(s\), got 0/) do
        run("((lambda (a . rest) a))")
      end
    end
  end
  ...
  describe "named let" do
    ...
    it "does not leak the loop binding into the outer scope" do
      expect_raises(Creme::SchemeRuntimeError, /unbound variable: loop/) do
    ...
    it "raises on a malformed binding" do
      expect_raises(Creme::SchemeRuntimeError, /let: bad binding/) do
        run("(let loop ((i)) i)")
      end
    end

    it "raises on an empty body" do
      expect_raises(Creme::SchemeRuntimeError, /let: named let body is empty/) do
        run("(let loop ())")
      end
    end
  end
  ...
  describe "do" do
    ...
    it "raises on a malformed binding" do
      expect_raises(Creme::SchemeRuntimeError, /do: bad binding/) do
        run("(do ((i 0 1 2)) (#t i))")
      end
    end

    it "raises on an empty test clause" do
      expect_raises(Creme::SchemeRuntimeError, /do: empty test clause/) do
        run("(do ((i 0)) ())")
      end
    end
  end
  ...
  describe "tail calls" do
    ...
    it "raises recursion depth exceeded for deep non-tail recursion" do
      expect_raises(Creme::SchemeRuntimeError, /recursion depth exceeded/) do
  ...
  describe "max_eval_depth" do
    it "defaults to DEFAULT_MAX_EVAL_DEPTH" do
      Creme::Interpreter.new(library_search_path: ["./modules"]).max_eval_depth.should eq(Creme::Interpreter::DEFAULT_MAX_EVAL_DEPTH)
    end

    it "is configurable via the constructor" do
      Creme::Interpreter.new(library_search_path: ["./modules"], max_eval_depth: 10).max_eval_depth.should eq(10)
    end
    ...
    it "raises sooner when configured with a smaller depth" do
      ...
      expect_raises(Creme::SchemeRuntimeError, /recursion depth exceeded/) do
        Creme.run_source(interp, "(define (f n) (+ 1 (f (+ n 1)))) (f 0)")
      end
    end
    ...
    it "raises for non-tail recursion that exceeds the default depth" do
      interp = Creme::Interpreter.new(library_search_path: ["./modules"])
      expect_raises(Creme::SchemeRuntimeError, /recursion depth exceeded/) do
        Creme.run_source(interp, "(define (count-up n) (if (= n 0) 0 (+ 1 (count-up (- n 1))))) (count-up 5000)")
      end
    end

    it "allows deeper non-tail recursion when configured with a larger depth" do
      interp = Creme::Interpreter.new(library_search_path: ["./modules"], max_eval_depth: 6000)
      Creme.run_source(interp, "(define (count-up n) (if (= n 0) 0 (+ 1 (count-up (- n 1))))) (count-up 5000)")
        .as(Creme::SchemeInt).value.should eq(5000_i64)
    end
  end

  describe "max_steps" do
    it "defaults to nil (unbounded)" do
      Creme::Interpreter.new(library_search_path: ["./modules"]).max_steps.should be_nil
    end

    it "is configurable via the constructor" do
      Creme::Interpreter.new(library_search_path: ["./modules"], max_steps: 10).max_steps.should eq(10)
    end
    ...
    it "raises SchemeExecutionLimitError for an infinite tail loop when set" do
      interp = Creme::Interpreter.new(library_search_path: ["./modules"], max_steps: 1000)
      expect_raises(Creme::SchemeExecutionLimitError, /execution step limit exceeded/) do
        Creme.run_source(interp, "(define (f) (f)) (f)")
      end
    end

    it "does not raise for a normal script well under the limit" do
      interp = Creme::Interpreter.new(library_search_path: ["./modules"], max_steps: 100_000)
      Creme.run_source(interp, "(define (sq x) (* x x)) (sq 5)").as(Creme::SchemeInt).value.should eq(25_i64)
    end

    it "does not interfere with max_eval_depth's own non-tail guard" do
      interp = Creme::Interpreter.new(library_search_path: ["./modules"], max_eval_depth: 10, max_steps: 1_000_000)
    ...
    it "resets its budget for each independent top-level call" do
      interp = Creme::Interpreter.new(library_search_path: ["./modules"], max_steps: 50)
      Creme.run_source(interp, "(+ 1 1)")
      Creme.run_source(interp, "(+ 2 2)").as(Creme::SchemeInt).value.should eq(4_i64)
    end
  end

  describe ".sandboxed" do
    it "denies all library imports by default" do
      Creme::Interpreter.sandboxed.allowed_libraries.should eq([] of String)
    end

    it "sets a finite default max_steps" do
      Creme::Interpreter.sandboxed.max_steps.should eq(100_000)
    end

    it "captures stdout by default instead of using the real STDOUT" do
      Creme::Interpreter.sandboxed.stdout.should be_a(IO::Memory)
    end

    it "denies even (scheme base) by default: guest code must import everything itself" do
      interp = Creme::Interpreter.sandboxed
      expect_raises(Creme::SchemeRuntimeError, /unbound variable: \+/) do
        Creme.run_source(interp, "(+ 1 2)")
      end
    end

    it "allows (scheme base) to be auto-bound if explicitly opted into via auto_import_base: true" do
      interp = Creme::Interpreter.sandboxed(auto_import_base: true)
    ...
    it "still allows widening allowed_libraries explicitly" do
      interp = Creme::Interpreter.sandboxed(allowed_libraries: ["scheme inexact"])
      Creme.run_source(interp, "(import (scheme inexact)) (sin 0)").as(Creme::SchemeFloat).value.should eq(0.0)
    end

    it "denies a library not in the explicit allowlist" do
      interp = Creme::Interpreter.sandboxed(allowed_libraries: ["scheme inexact"])
      expect_raises(Creme::SchemeRuntimeError, /import: library \(creme process\) is not permitted/) do
        Creme.run_source(interp, "(import (creme process))")
      end
    end
    ...
    it "denies (creme ffi) like any other library not in the explicit allowlist" do
      interp = Creme::Interpreter.sandboxed(allowed_libraries: ["scheme inexact"])
      expect_raises(Creme::SchemeRuntimeError, /import: library \(creme ffi\) is not permitted/) do
        Creme.run_source(interp, "(import (creme ffi))")
      end
    end
  end
  ...
  describe "delay/force" do
    ...
    it "raises for the wrong number of arguments" do
      expect_raises(Creme::SchemeRuntimeError, /delay: expects 1 argument/) do
        run("(delay)")
      end
    end
  end
  ...
  describe "#apply" do
    it "applies a Builtin" do
      interp = Creme::Interpreter.new(library_search_path: ["./modules"])
      f = interp.global.get("+")
      interp.apply(f, [Creme::SchemeInt.new(1_i64), Creme::SchemeInt.new(2_i64)] of Creme::SchemeValue)
        .as(Creme::SchemeInt).value.should eq(3_i64)
    end

    it "applies a Lambda" do
      interp = Creme::Interpreter.new(library_search_path: ["./modules"])
      Creme.run_source(interp, "(define (sq x) (* x x))")
      f = interp.global.get("sq")
      interp.apply(f, [Creme::SchemeInt.new(4_i64)] of Creme::SchemeValue)
        .as(Creme::SchemeInt).value.should eq(16_i64)
    end

    it "raises when the callee isn't applicable" do
      expect_raises(Creme::SchemeRuntimeError, /not applicable/) do
        run("(1 2 3)")
      end
    end
  end
  ...
  describe "#parse_formals" do
    it "parses fixed params" do
      interp = Creme::Interpreter.new(library_search_path: ["./modules"])
      params, rest = interp.parse_formals(Creme.a_to_list([Creme::SchemeSym.of("a"), Creme::SchemeSym.of("b")] of Creme::SchemeValue))
    ...
    it "parses a fully variadic symbol spec" do
      interp = Creme::Interpreter.new(library_search_path: ["./modules"])
      params, rest = interp.parse_formals(Creme::SchemeSym.of("args"))
    ...
    it "parses a dotted rest param" do
      interp = Creme::Interpreter.new(library_search_path: ["./modules"])
      formals = Creme.a_to_list([Creme::SchemeSym.of("a")] of Creme::SchemeValue, Creme::SchemeSym.of("rest"))
    ...
    it "raises for a non-symbol formal" do
      interp = Creme::Interpreter.new(library_search_path: ["./modules"])
      formals = Creme.a_to_list([Creme::SchemeInt.new(1_i64)] of Creme::SchemeValue)
      expect_raises(Creme::SchemeRuntimeError, /bad formal parameter/) do
