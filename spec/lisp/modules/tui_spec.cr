...
private def w(src : String) : String
  ...
  LISP.run_source(interp, "(require 'tui) #{src}").write_string
end
...
describe "tui module" do
  ...
  it "raises for an out-of-range color index" do
    expect_raises(LISP::LispRuntimeError, /tui:color-index:/) { w("(tui:color-index 999)") }
  end

  it "raises for an unknown named color" do
    expect_raises(LISP::LispRuntimeError, /tui:color-named: unknown color name/) { w("(tui:color-named 'periwinkle)") }
  end
  ...
  it "raises for an unknown style key" do
    expect_raises(LISP::LispRuntimeError, /tui:style: unknown style key/) do
  ...
  it "raises when the highlighter-fn returns something other than a list of (text . style) pairs" do
    expect_raises(LISP::LispRuntimeError, /tui:text-edit-set-highlighter! highlighter-fn: expected a list of \(text . style\) pairs/) do
      w(<<-LISP)
        (define pane (tui:text-edit "hi"))
  ...
  it "raises when text-edit-set-highlighter! is given a non-text-edit scrollable" do
    expect_raises(LISP::LispRuntimeError, /tui:text-edit-set-highlighter!: expected a tui:text-edit pane/) do
      w(<<-LISP)
        (define pane
  ...
  it "raises when text-edit-value is given a non-text-edit scrollable" do
    expect_raises(LISP::LispRuntimeError, /tui:text-edit-value: expected a tui:text-edit pane/) do
      w(<<-LISP)
        (define pane
          (tui:make-scrollable (lambda () "") (lambda () 0) (lambda (buf) #f) (lambda (ev) #f) (lambda () "")))
  ...
  it "builds a vstack of two panes and can swap the bottom pane" do
    w(<<-LISP).should match(/#<tui-widget>/)
      (define screen (tui:screen))
  ...
  it "raises for an unknown key name" do
    expect_raises(LISP::LispRuntimeError, /tui:handle-key!: unknown key name/) do
      w(<<-LISP)
