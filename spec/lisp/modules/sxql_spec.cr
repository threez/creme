...
describe "sxql module" do
  it "builds a select with where/order-by/limit/offset" do
    w(<<-LISP).should eq(%(("SELECT id, name, age FROM person WHERE (age >= ? AND age < ?) ORDER BY age DESC LIMIT ? OFFSET ?" (18 65 10 5))))
  ...
  it "AND-composes repeated where clauses" do
    w(<<-LISP).should eq(%(("SELECT x FROM t WHERE (a = ? AND b = ?)" (1 2))))
      (sxql:yield (sxql:select '(x) (sxql:from 't) (sxql:where (sxql:= 'a 1)) (sxql:where (sxql:= 'b 2))))
      LISP
  end

  it "renders distinct, group-by and having" do
    w(<<-LISP).should eq(%(("SELECT DISTINCT customer, SUM(amount) AS total FROM orders GROUP BY customer HAVING SUM(amount) > ?" (20))))
  ...
  it "renders inner and left joins" do
    w(<<-LISP).should eq(%(("SELECT a.x, b.y FROM a INNER JOIN b ON a.id = b.a_id" ())))
      (sxql:yield (sxql:select '(a.x b.y) (sxql:from 'a) (sxql:join 'b (sxql:= 'a.id 'b.a_id))))
      LISP

    w(<<-LISP).should eq(%(("SELECT a.x FROM a LEFT JOIN b ON a.id = b.a_id" ())))
  ...
  it "composes and/or/not/in/not-in/like/is-null/is-not-null" do
    w(<<-LISP).should eq(%(("SELECT x FROM t WHERE (status IN (?, ?) OR NOT (y IS NULL))" ("a" "b"))))
      (sxql:yield (sxql:select '(x) (sxql:from 't)
                    (sxql:where (sxql:or (sxql:in 'status (list "a" "b"))
                                          (sxql:not (sxql:is-null 'y))))))
      LISP

    w(<<-LISP).should eq(%(("SELECT x FROM t WHERE (id NOT IN (?, ?) AND y IS NOT NULL)" (1 2))))
      (sxql:yield (sxql:select '(x) (sxql:from 't)
                    (sxql:where (sxql:and (sxql:not-in 'id (list 1 2)) (sxql:is-not-null 'y)))))
      LISP

    w(<<-LISP).should eq(%(("SELECT x FROM t WHERE name LIKE ?" ("%foo%"))))
      (sxql:yield (sxql:select '(x) (sxql:from 't) (sxql:where (sxql:like 'name "%foo%"))))
      LISP
  end

  it "renders a case expression" do
    w(<<-LISP).should eq(%(("SELECT CASE WHEN age > ? THEN ? ELSE ? END AS category FROM person" (18 "adult" "minor"))))
  ...
  it "builds insert-into with set=, on-conflict, and returning" do
    w(<<-LISP).should eq(%(("INSERT INTO person (name, age) VALUES (?, ?) RETURNING id" ("Alice" 30))))
      (sxql:yield (sxql:insert-into 'person (sxql:set= 'name "Alice" 'age 30) (sxql:returning 'id)))
      LISP

    w(<<-LISP).should eq(%(("INSERT INTO person (name) VALUES (?) ON CONFLICT (name) DO NOTHING" ("Alice"))))
      (sxql:yield (sxql:insert-into 'person (sxql:set= 'name "Alice") (sxql:on-conflict-do-nothing 'name)))
      LISP

    w(<<-LISP).should eq(%(("INSERT INTO person (name) VALUES (?) ON CONFLICT DO NOTHING" ("Alice"))))
      (sxql:yield (sxql:insert-into 'person (sxql:set= 'name "Alice") (sxql:on-conflict-do-nothing)))
      LISP

    w(<<-LISP).should eq(%(("INSERT INTO person (name) VALUES (?) ON CONFLICT (name) DO UPDATE SET name = ?" ("Alice" "Alice"))))
  ...
  it "builds update with set= and where" do
    w(<<-LISP).should eq(%(("UPDATE person SET age = ? WHERE name = ?" (31 "Alice"))))
  ...
  it "builds union and union-all" do
    w(<<-LISP).should eq(%(("SELECT x FROM a UNION SELECT x FROM b" ())))
      (sxql:yield (sxql:union (sxql:select '(x) (sxql:from 'a)) (sxql:select '(x) (sxql:from 'b))))
      LISP

    w(<<-LISP).should eq(%(("SELECT x FROM a UNION ALL SELECT x FROM b" ())))
  ...
  it "builds create-table with column constraints" do
    w(<<-LISP).should eq(%(("CREATE TABLE IF NOT EXISTS person (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE, age INTEGER DEFAULT 0)" ())))
  ...
  it "renders a string DEFAULT literal with quote-escaping" do
    w(<<-LISP).should eq(%(("CREATE TABLE t (label TEXT DEFAULT 'it''s')" ())))
  ...
  it "builds create-index and drop-index" do
    w(<<-LISP).should eq(%(("CREATE UNIQUE INDEX IF NOT EXISTS idx_person_name ON person (name)" ())))
  ...
  it "raises a LispRuntimeError on a malformed clause" do
    expect_raises(LISP::LispRuntimeError, /sxql:select: expected a clause/) do
      run(%((sxql:select '(id) 42)))
    end
  end

  it "raises a LispRuntimeError when yielding a non-statement" do
    expect_raises(LISP::LispRuntimeError, /sxql:yield: expected a built statement/) do
      run(%((sxql:yield 42)))
    end
  end

  it "raises a LispRuntimeError on odd set= arguments" do
    expect_raises(LISP::LispRuntimeError, /sxql:set=: expected an even number/) do
      run(%((sxql:set= 'a 1 'b)))
    end
  end

  it "raises a LispRuntimeError on an unknown column constraint" do
    expect_raises(LISP::LispRuntimeError, /sxql:column: unknown constraint/) do
