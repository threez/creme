...
describe "defmacro" do
  ...
  it "raises when a macro is passed to the apply builtin" do
    expect_raises(LISP::LispRuntimeError, /macro cannot be applied as a procedure: m/) do
  ...
  describe "malformed input" do
    it "raises when the name is missing" do
      expect_raises(LISP::LispRuntimeError, /defmacro: malformed/) { run("(defmacro)") }
    end

    it "raises when the name isn't a symbol" do
      expect_raises(LISP::LispRuntimeError, /defmacro: macro name must be a symbol/) { run("(defmacro 1 (x) x)") }
    end

    it "raises when the formals list is missing" do
      expect_raises(LISP::LispRuntimeError, /defmacro: malformed/) { run("(defmacro m)") }
    end

    it "raises when the body is empty" do
      expect_raises(LISP::LispRuntimeError, /defmacro: macro body is empty/) { run("(defmacro m (x))") }
    end

    it "raises for a bad formal parameter" do
      expect_raises(LISP::LispRuntimeError, /bad formal parameter/) { run("(defmacro m (1) 1)") }
    end
  end

  it "keeps dispatching to the if special form even when a macro named if is defined" do
    w("(defmacro if (a) a) (if #t 1 2)").should eq("1")
  end
  ...
  it "supports local, nested macro definitions scoped to their let" do
    ...
    expect_raises(LISP::LispRuntimeError, /unbound variable: m/) do
