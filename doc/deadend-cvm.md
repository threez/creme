# cvm dead-ends — explored and rejected

Companion to `doc/optimization-cvm.md` (which covers the optimizations that
landed). This file collects cvm optimization ideas that were explored and
...
## Where cvm stands vs Guile (why the interpreter tail is exhausted)

The cross-language bench puts cvm within ~1.1× of Guile 3.0.11 overall, but
...
is worth **tak 2.3×, fib 2.8×, nqueens 3.2×**. Back that out and cvm's
...
- **`SelfTailCall` opcode that skips the `upvalue_get`.** The Section-7
  ...
  discover it's a self-call. A runtime-quickened cvm-only `OP_SELFTAILCALL`
...
- **Fused `RecRef`/`RecMake` record opcodes.** Give record accessors/
  ...
  version of record-test runs **1.56×** faster), but: (1) cvm's accessor
...
- **`GC_ENABLE_INCREMENTAL=1` (Boehm's incremental/generational mode).**
  Tested alongside `doc/optimization-cvm.md`'s Section 8 GC-heap-sizing work,
