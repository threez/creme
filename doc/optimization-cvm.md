# The performance journey — cvm, the standalone C11 VM
...
This document covers optimization work specific to cvm's own
...
- `doc/optimization-general.md` — techniques shared by (or ported across)
  ...
  in cvm as part of that shared work (counted-loop fusion, its
...
and where cvm stands against Guile (its JIT, and why the interpreter-level
tail is exhausted) — see `doc/deadend-cvm.md`. Read it before starting new
...
cvm represents every Scheme value as a single `Value` struct: a type tag
...
cvm is passed by value constantly — through the dispatch loop, into every
...
with no correctness regressions. This was a `cvm/*.c`-only change; the
...
Benchmarking cvm against a reference Lua interpreter identified several
workloads where cvm measurably trailed despite winning overall on the
...
- **The integer fast path inside the numeric helper functions wasn't
  ...
  *whole* function — including its float/rational/complex fallback
  ...
  dispatch loop (see `doc/optimization-crystal.md`, Section 6); cvm's C
...
than it. Across the whole suite, cvm's total time relative to that same
...
cvm's self-hosted bootstrap compiler (`modules/creme/compiler/
...
place (self-modifying bytecode, kept safe under cvm's actor model, where
the same `Chunk`'s instructions can be shared and executed concurrently
across pthreads — see `cvm/actor.c` — via a single relaxed atomic store:
...
of six new `OP_QCALLGLOBAL_*` opcodes (`cvm/opcodes.h`, appended after
...
`RecordCallable` of kind `RC_ACCESSOR` (`cvm/value.h`), built by
`build_record_bindings` (`cvm/vm.c`). A call like `(point-x p)`
...
call to a new `OP_QCALLGLOBAL_RECACC` opcode (`cvm/opcodes.h`,
...
`competition/bench.scm`'s cross-language suite — this is a cvm-only
...
compiled `.cvmc`, only `cvm/cvm` itself rebuilt in between): steady-
...
chunk). Verified against the full cvm spec suite and the `examples/*.scm`
...
Profiling `nqueens` — the workload where cvm trails the other bytecode VMs
...
everything else is untouched. This is entirely cvm-side — no bytecode or
...
critical path), so it's a documented dead end, see `doc/deadend-cvm.md`.

Correctness is the gate here, since this is the single hottest VM path: the
full cvm spec suite runs unchanged (same pre-existing failures, zero new),
...
Every allocation in cvm goes through Boehm's `GC_MALLOC` (see `doc/
...
`competition/scheme/bench/creme.scm`'s 9-workload suite through `cvm/cvm`
...
this costs nothing even for cvm's long-running-server use (`mux-listen!`'s
