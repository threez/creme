...
Companion to `doc/deadend-cvm.md` (VM/opcode-level dead-ends) and
`doc/optimization-cvm.md` (the wins). This file covers a different axis:
compiler-flag tuning on `cvm/Makefile` and `competition/c/demo-todo/Makefile`,
prompted by the todo-app HTTP bench (`competition/bench.scm`) putting cvm and
...
`cvm/Makefile` stays at its original `-O2 -g -fno-omit-frame-pointer`, and
...
- **cvm `release` target: `-O3` instead of `-O2`.** Naive reading of
  Crystal's `--release` as "always the max opt level" — tried bumping cvm's
  default `-O2` to `-O3`. **Rejected before even landing**: this project's
  own prior experience already measured `-O3` on cvm's dispatch loop as
...
- **cvm `release` target: `-O2 -DNDEBUG`, dropping `-g`/
  ...
  `cvm/*.c` to compile out. Dropping `-g`/`-fno-omit-frame-pointer` also
  ...
  regardless of `-g`, since `cvm/Makefile`'s `LDFLAGS := -rdynamic` is
  unconditional), and `backtrace(3)` unwinds via glibc's `.eh_frame` CFI,
  not frame pointers — so cvm's own profiler works the same either way.
  ...
  **Reverted** — `cvm/Makefile` has no `release` target; `all` stays the
...
Neither avoiding `-O3` on cvm, stripping `-g`, nor `-DNDEBUG` on either C
binary moved todo-app throughput outside normal run-to-run noise. The gap
between cvm/C and Crystal's Kemal+Granite+ECR twin on this benchmark isn't a
compiler-flag artifact — it's structural (templating/ORM-layer differences),
and not something this axis of tuning can close. Building cvm/C at a genuine
