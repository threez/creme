/* libFuzzer harness for cvm_load_from_bytes (loader.c) -- the SCB1
 * bytecode deserializer, the concrete untrusted-input boundary an
 * embedder crosses whenever it loads a precompiled .cvmc/bytevector it
 * didn't itself just compile (see cvm/README.md's embedding notes).
 * Every OTHER cvm entry point (raw .scm source) goes through the self-
 ...
 *
 * Build: `gmake -C cvm fuzz` (needs clang; libFuzzer ships with clang's
 ...
 * or seed it yourself first from any real .cvmc files you have locally
 * (e.g. `gmake -C cvm all` produces cvm/compiler-run.cvmc, or
 * `bin/creme --emit-cvm <script.scm> <out.cvmc>` on any script). Example:
 * `mkdir -p cvm/fuzz/corpus && cvm/fuzz-loader -max_total_time=300
 * cvm/fuzz/corpus`. Add `-jobs=N`/`-workers=N` for a parallel run, or a
 ...
 * "cvm: corrupt bytecode..." abort must be caught right here, not tear
