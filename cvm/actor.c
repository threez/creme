 ...
 * vm.c's cvm_new_child_vm) running on its own pthread; cvm's bytecode dispatch
 ...
 * threads, which is the only cvm-specific thread-safety property this
 ...
 * total isolation with no cvm code involved at all): on this project's
 ...
 * Interpreter, threaded through as `current_interp.actor_system`. cvm has
 ...
 * native's own cached-Channel-plus-dedicated-writer-fiber design, cvm's
...
typedef struct ActorContext {
  char *id; /* GC_MALLOC'd, nul-terminated -- a plain sequential counter
             * here (cvm's own id scheme, not necessarily matching
             * native's own id format -- nothing depends on the two
             * agreeing, ids are only ever compared within cvm's own
             * registry). */
  VM *vm;
 ...
 * case; cvm never needs the split for 'local since that's always
 ...
 * node's own registry -- cvm never needs a "remote-shaped" ref for
...
/* <down> record type -- built via cvm's existing RecordType/SchemeRecord
 ...
 * reuses its full Scheme reader/write_string; cvm has no native C-level
 * Scheme reader to reuse) -- cvm-to-cvm distribution works correctly,
