...
static Value bi_load_chunk_bytes(VM *vm, Value *args, int nargs) {
   ...
   * cvm's own "compiler mode" (main.c), where main() itself only ever
   * registers builtins based on precompiled compiler-run.cvmc's own
 ...
 * see cvm/compiler-run.scm's own `environment`/`null-environment`/`eval`
 ...
 * environment (cvm/compiler-run.scm) -- once Scheme-side import-set
 ...
 * (`and`/`or`/`when`/...), which cvm never binds as a global at all
 ...
 * except filtering actually left OUT -- see cvm/compiler-run.scm's
 ...
 * the given SCB1 bytes against env-box's own separate VM instead of the
 ...
 * the one real global table (cvm/README.md's own previous "eval ignores
