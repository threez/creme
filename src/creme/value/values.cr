...
module Creme
  ...
  class Cons
    ...
    private def write_seq(io : IO, write_mode : Bool) : Nil
      if Creme.write_seq_ancestor?(object_id)
        io << "..."
        return
      end
      io << '('
      ...
      begin
        while cur.is_a?(Cons)
          id = cur.object_id
          if Creme.write_seq_ancestor?(id)
            io << (first ? "..." : " ...")
            cur = NIL
            break
          end
          Creme.mark_write_seq_ancestor(id)
          pushed << id
          io << ' ' unless first
          first = false
          if write_mode
        ...
        pushed.each { |pid| Creme.unmark_write_seq_ancestor(pid) }
      end
      io << ')'
    end
  end
  ...
  class SchemeVector
    ...
    private def write_seq(io : IO, write_mode : Bool) : Nil
      ...
      begin
        ...
        Creme.unmark_write_seq_ancestor(id)
      end
      io << ')'
    end
  end
