...
# (Creme::Csv, below) -- no external shard/vendored dependency. This used
...
# booleans and hand them to Creme::Csv::Builder as native Crystal values so
...
# to exactly what `Creme::Builtins::CsvLibrary` below needs.
module Creme::Csv
  DEFAULT_SEPARATOR  = ','
  DEFAULT_QUOTE_CHAR = '"'
...
module Creme::Builtins::CsvLibrary
  extend self
  include Creme::BuiltinHelpers

  @[Creme::SchemeFn("csv-read", min: 1, max: 3)]
  ...
  @[Creme::SchemeFn("csv-write", min: 1, max: 3)]
  def csv_write(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    text = String.build do |io|
      builder = Creme::Csv::Builder.new(io, sep, quoting: quoting)
      rows.each { |row| csv_write_row(builder, row, "csv-write") }
    end
    SchemeStr.new(text)
  end

  @[Creme::SchemeFn("csv-write-headers", min: 2, max: 4)]
  ...
  @[Creme::SchemeFn("csv-writer-open", min: 1, max: 3)]
  def csv_writer_open(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    SchemeBox.new("csv-writer", Creme::Csv::Builder.new(port.io, sep, quoting: quoting), "#<csv-writer>")
  end

  @[Creme::SchemeFn("csv-writer-row!", min: 1, max: -1)]
  ...
  @[Creme::SchemeFn("csv-writer?", min: 1, max: 1)]
  ...
  @[Creme::SchemeFn("csv-reader-open", min: 1, max: 4)]
  def csv_reader_open(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    SchemeBox.new("csv-reader", Creme::Csv::Parser.new(port.io, sep, quote, chunk_size), "#<csv-reader>")
  end

  @[Creme::SchemeFn("csv-reader-read!", min: 1, max: 1)]
  ...
  @[Creme::SchemeFn("csv-reader?", min: 1, max: 1)]
  ...
  private def csv_write_row(csv : Creme::Csv::Builder, cells : Array(SchemeValue), who : String) : Nil
    csv.row do |row|
  ...
  private def csv_row_cells(v : SchemeValue, who : String) : Array(SchemeValue)
    return Creme.list_to_a(v) if Creme.proper_list?(v)
  ...
  private def csv_chunk_size_arg(args : Array(SchemeValue), index : Int32, who : String) : Int32
    return Creme::Csv::DEFAULT_CHUNK_SIZE if args.size <= index
  ...
  private def csv_quoting_arg(args : Array(SchemeValue), index : Int32, who : String) : Creme::Csv::Builder::Quoting
    return Creme::Csv::Builder::Quoting::RFC if args.size <= index
    v = args[index]
    raise SchemeRuntimeError.new("#{who}: expected a quoting symbol, got #{v.write_string}") unless v.is_a?(SchemeSym)
    case v.name
    when "none" then Creme::Csv::Builder::Quoting::NONE
    when "rfc"  then Creme::Csv::Builder::Quoting::RFC
    when "all"  then Creme::Csv::Builder::Quoting::ALL
    else
      raise SchemeRuntimeError.new("#{who}: unknown quoting mode '#{v.name}' (expected none, rfc, or all)")
    end
  end

  private def csv_writer_arg(v : SchemeValue, who : String) : Creme::Csv::Builder
    raise SchemeRuntimeError.new("#{who}: expected a csv writer, got #{v.write_string}") unless v.is_a?(SchemeBox) && v.tag == "csv-writer"
    v.get(Creme::Csv::Builder)
  end

  private def csv_reader_arg(v : SchemeValue, who : String) : Creme::Csv::Parser
    raise SchemeRuntimeError.new("#{who}: expected a csv reader, got #{v.write_string}") unless v.is_a?(SchemeBox) && v.tag == "csv-reader"
    v.get(Creme::Csv::Parser)
  end
end

module Creme
  class Interpreter
    register_library ["creme", "builtin", "csv"], Creme::Builtins::CsvLibrary
  end
end
