...
# need. Backed by a real Crystal Hash from Creme.scheme_hash(key) to a
...
# way Scheme code expects), hence the bucket-of-Creme.scheme_equal?
  ...
  class SchemeHashTable
    ...
    def set(key : SchemeValue, value : SchemeValue) : Nil
      @mutex.synchronize do
        bucket = @buckets[Creme.scheme_hash(key)] ||= [] of {SchemeValue, SchemeValue}
    ...
    def get?(key : SchemeValue) : SchemeValue?
      @mutex.synchronize do
        bucket = @buckets[Creme.scheme_hash(key)]?
    ...
    private def unsafe_index_in(bucket : Array({SchemeValue, SchemeValue}), key : SchemeValue) : Int32?
      bucket.each_with_index do |(k, _), i|
        return i if Creme.scheme_equal?(k, key)
      end
      nil
    end
...
module Creme::Builtins::HashTable
  extend self
  include Creme::BuiltinHelpers

  @[Creme::SchemeFn("make-hash-table", min: 0, max: 0)]
  ...
  @[Creme::SchemeFn("hash-table-set!", min: 3, max: 3)]
  ...
  @[Creme::SchemeFn("hash-table-ref", min: 2, max: 3)]
  ...
  @[Creme::SchemeFn("hash-table-delete!", min: 2, max: 2)]
  ...
  @[Creme::SchemeFn("hash-table-contains?", min: 2, max: 2)]
  ...
  @[Creme::SchemeFn("hash-table-keys", min: 1, max: 1)]
  def hash_table_keys(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    h = hash_table_arg(args[0], "hash-table-keys")
    Creme.a_to_list(h.snapshot.map { |(k, _)| k })
  end

  @[Creme::SchemeFn("hash-table-values", min: 1, max: 1)]
  def hash_table_values(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    Creme.a_to_list(h.snapshot.map { |(_, v)| v })
  end

  @[Creme::SchemeFn("hash-table->alist", min: 1, max: 1)]
  def hash_table_to_alist(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    Creme.a_to_list(h.snapshot.map { |(k, v)| Cons.new(k, v).as(SchemeValue) })
  end
...
module Creme
  class Interpreter
    register_library ["creme", "builtin", "hash-table"], Creme::Builtins::HashTable
  end
end
