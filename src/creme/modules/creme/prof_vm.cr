...
# src/creme/eval/interpreter.cr) that periodically inspects the instruction
# currently at the top of the VM's fetch-decode-dispatch loop (VM#execute,
# src/creme/eval/vm.cr) — not from a signal handler. Reading interpreter
...
# src/creme.cr's require for the two files' differing guards, and
...
module Creme::Builtins::ProfVM
  ...
  @[Creme::SchemeFn("profile-scheme", min: 1, max: 2)]
  ...
  @[Creme::SchemeFn("profile-scheme-report?", min: 1, max: 1)]
  ...
  @[Creme::SchemeFn("profile-scheme-total-samples", min: 1, max: 1)]
  ...
  @[Creme::SchemeFn("profile-scheme-top", min: 2, max: 2)]
...
module Creme
  class Interpreter
    register_library ["creme", "builtin", "prof-vm"], Creme::Builtins::ProfVM
  end
end
