...
module Creme::Builtins::SqlLibrary
  extend self
  ...
  @[Creme::SchemeFn("sql-open", min: 1, max: -1)]
  def sql_open(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    conn =
      if path == ":memory:"
        ...
        Creme::SqlConnection.new(db, db)
      else
        Creme::SqlConnection.new(DB.open(writer_uri(path, max_writers)), DB.open(reader_uri(path, max_readers)))
      end
  ...
  @[Creme::SchemeFn("sql-close", min: 1, max: 1)]
  ...
  @[Creme::SchemeFn("sql-execute", min: 2, max: -1)]
  ...
  @[Creme::SchemeFn("sql-query", min: 2, max: -1)]
  ...
  @[Creme::SchemeFn("sql-scalar", min: 2, max: -1)]
  ...
  @[Creme::SchemeFn("csv-import!", min: 3, max: -1)]
  def csv_import(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    io = File.open(path)
    begin
      parser = Creme::Csv::Parser.new(io, opts[:separator], opts[:quote], opts[:chunk_size])
  ...
  private def csv_import_columns_and_types(parser : Creme::Csv::Parser, opts) : {Array(String), Array(String)}
  ...
  private def csv_import_columns_arg(v : SchemeValue, who : String, keyword : String) : Array({String, String})
    Creme.list_to_a(v).map do |entry|
  ...
  @[Creme::SchemeFn("sql-connection?", min: 1, max: 1)]
...
module Creme
  class Interpreter
    register_library ["creme", "builtin", "sql"], Creme::Builtins::SqlLibrary
  end
end
