...
class HTTP::Server::Context
  ...
  property mux_interp : Creme::Interpreter?
end
...
module Creme
  ...
  class MuxMiddlewareHandler
    ...
    def call(context : HTTP::Server::Context) : Nil
      request = Creme::Builtins::MuxLibrary.request_to_scheme(context)
      request_interp = Creme::Builtins::MuxLibrary.request_interpreter(context, @interp)
      called_next = false
      ...
      Creme::Builtins::MuxLibrary.write_response(request_interp, context, result, "mux-use!") unless called_next
    end
  end
end

module Creme::Builtins::MuxLibrary
  extend self
  include Creme::BuiltinHelpers

  @[Creme::SchemeFn("mux-router", min: 0, max: 0)]
  def mux_router(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    SchemeBox.new("mux-router", Creme::MuxApp.new(Mux::Router.new), "#<mux-router>")
  end

  @[Creme::SchemeFn("mux-router?", min: 1, max: 1)]
  ...
  {% for method in ["get", "head", "post", "put", "delete", "patch"] %}
    @[Creme::SchemeFn("mux-{{method.id}}!", min: 3, max: 3)]
  ...
  @[Creme::SchemeFn("mux-use!", min: 2, max: 2)]
  ...
  @[Creme::SchemeFn("mux-listen!", min: 2, max: 3)]
  def mux_listen(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    handlers = [Creme::MuxInterpreterPoolHandler.new(interp).as(HTTP::Handler)]
    handlers.concat(app.middlewares.map { |middleware| Creme::MuxMiddlewareHandler.new(interp, middleware).as(HTTP::Handler) })
  ...
  @[Creme::SchemeFn("mux-address", min: 1, max: 1)]
  ...
  @[Creme::SchemeFn("mux-base-url", min: 1, max: 1)]
  ...
  @[Creme::SchemeFn("mux-close!", min: 1, max: 1)]
  ...
  # Public (not private): also called by Creme::MuxMiddlewareHandler, a
  ...
  def request_to_scheme(context : HTTP::Server::Context) : SchemeValue
    ...
    Creme.a_to_list([
      ...
      Cons.new(SchemeStr.new("path-params"), Creme.a_to_list(param_pairs)).as(SchemeValue),
      Cons.new(SchemeStr.new("headers"), Creme.a_to_list(header_pairs)).as(SchemeValue),
  ...
  def write_response(interp : Interpreter, context : HTTP::Server::Context, response : SchemeValue, who : String) : Nil
    ...
    native = Creme.from_scheme(body_proc ? alist_without(response, "body") : response)
      .as(Hash(String, Creme::Convertible))
    context.response.status_code = (native["status"]? || 200_i64).as(Int64).to_i32
    if headers = native["headers"]?
      headers.as(Hash(String, Creme::Convertible)).each do |name, value|
  ...
  private def alist_lookup(alist : SchemeValue, key : String) : SchemeValue?
    Creme.list_to_a(alist).each do |pair|
  ...
  private def alist_without(alist : SchemeValue, key : String) : SchemeValue
    pairs = Creme.list_to_a(alist).reject do |pair|
  ...
  private def mux_router_arg(v : SchemeValue, who : String) : Creme::MuxApp
...
module Creme
  class Interpreter
    register_library ["creme", "builtin", "mux"], Creme::Builtins::MuxLibrary
  end
end
