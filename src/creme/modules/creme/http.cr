...
module Creme::Builtins::HttpLibrary
  ...
  @[Creme::SchemeFn("http-request", min: 2, max: 4)]
  ...
  private def http_headers_arg(v : SchemeValue, who : String) : HTTP::Headers
    raise SchemeRuntimeError.new("#{who}: expected headers alist, got #{v.write_string}") unless Creme.proper_list?(v)
...
module Creme
  class Interpreter
    register_library ["creme", "builtin", "http"], Creme::Builtins::HttpLibrary
  end
end
