...
module Creme::Builtins::ProcessLibrary
  extend self
  ...
  @[Creme::SchemeFn("command-line", min: 0, max: 0)]
  def command_line(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    Creme.a_to_list(([PROGRAM_NAME] + ARGV).map { |arg| SchemeStr.new(arg).as(SchemeValue) })
  end
end

# creme-only process control, beyond R7RS's (scheme process-context) contract.
module Creme::Builtins::ProcessExtra
  extend self
  include Creme::BuiltinHelpers

  @[Creme::SchemeFn("process-run", min: 2, max: 2)]
  def process_run(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    cmd_args = Creme.list_to_a(args[1]).map do |elem|
  ...
  private def spawn_env_alist(v : SchemeValue, who : String) : Hash(String, String)
    raise SchemeRuntimeError.new("#{who}: expected 'env alist, got #{v.write_string}") unless Creme.proper_list?(v)
    env = {} of String => String
    Creme.list_to_a(v).each do |entry|
  ...
  @[Creme::SchemeFn("process-alive?", min: 1, max: 1)]
  ...
  @[Creme::SchemeFn("process-kill!", min: 1, max: -1)]
  ...
  @[Creme::SchemeFn("process-write-line!", min: 2, max: 2)]
  ...
  # child.
  @[Creme::SchemeFn("process-wait!", min: 1, max: 1)]
  ...
  # caller of this.
  @[Creme::SchemeFn("sleep-ms!", min: 1, max: 1)]
  ...
  class Interpreter
    register_library ["creme", "builtin", "process"] do |env|
      register_module(Creme::Builtins::ProcessLibrary, env) +
        register_module(Creme::Builtins::ProcessExtra, env)
    end
  end
end
