...
module Creme::Builtins::RegexLibrary
  extend self
  include Creme::BuiltinHelpers

  @[Creme::SchemeFn("regexp", min: 1, max: 1)]
  ...
  @[Creme::SchemeFn("regexp-matches?", min: 2, max: 2)]
  ...
  @[Creme::SchemeFn("regexp-search", min: 2, max: 2)]
  def regexp_search(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    if found = rx.match(s)
      Creme.a_to_list(found.to_a.map { |group| group ? SchemeStr.new(group).as(SchemeValue) : FALSE.as(SchemeValue) })
    else
      FALSE.as(SchemeValue)
    end
  end

  @[Creme::SchemeFn("regexp-extract", min: 2, max: 2)]
  def regexp_extract(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    s.scan(rx) do |found|
      matches << Creme.a_to_list(found.to_a.map { |group| group ? SchemeStr.new(group).as(SchemeValue) : FALSE.as(SchemeValue) })
    end
    Creme.a_to_list(matches)
  end

  @[Creme::SchemeFn("regexp-replace", min: 3, max: 3)]
  ...
  @[Creme::SchemeFn("regexp-replace-all", min: 3, max: 3)]
  ...
  @[Creme::SchemeFn("regexp-split", min: 2, max: 2)]
  def regexp_split(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    Creme.a_to_list(s.split(rx).map { |x| SchemeStr.new(x).as(SchemeValue) })
  end
...
module Creme
  class Interpreter
    register_library ["creme", "builtin", "regex"], Creme::Builtins::RegexLibrary
  end
end
