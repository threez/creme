...
module Creme::R7RS::Vectors
  ...
  @[Creme::SchemeFn("vector-ref", min: 2, max: 2)]
  ...
  @[Creme::SchemeFn("vector-set!", min: 3, max: 3)]
  ...
  @[Creme::SchemeFn("vector-length", min: 1, max: 1)]
  ...
  def vector_to_list(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    Creme.a_to_list(elems[first...last])
  end

  @[Creme::SchemeFn("list->vector", min: 1, max: 1)]
  def list_to_vector(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    SchemeVector.new(Creme.list_to_a(args[0]))
  end

  @[Creme::SchemeFn("vector-map", min: 2, max: -1)]
  ...
  @[Creme::SchemeFn("vector-for-each", min: 2, max: -1)]
  ...
  @[Creme::SchemeFn("vector-copy", min: 1, max: 3)]
  ...
  @[Creme::SchemeFn("vector-copy!", min: 3, max: 5)]
  ...
  @[Creme::SchemeFn("vector-fill!", min: 2, max: 4)]
  ...
  @[Creme::SchemeFn("vector-append", min: 0, max: -1)]
...
module Creme
  class Interpreter
    private def install_vectors(env : Env) : Array(String)
      register_module(Creme::R7RS::Vectors, env)
    end
  end
end
