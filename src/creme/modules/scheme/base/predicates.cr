...
module Creme::R7RS::Predicates
  extend self
  include Creme::BuiltinHelpers

  @[Creme::SchemeFn("number?", min: 1, max: 1)]
  ...
  @[Creme::SchemeFn("real?", min: 1, max: 1)]
  ...
  @[Creme::SchemeFn("rational?", min: 1, max: 1)]
  def rational_p(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    v = args[0]
    SchemeBool.of(Creme.exact?(v) || (v.is_a?(SchemeFloat) && v.value.finite?))
  end
  ...
  @[Creme::SchemeFn("integer?", min: 1, max: 1)]
  ...
  @[Creme::SchemeFn("zero?", min: 1, max: 1)]
  ...
  @[Creme::SchemeFn("positive?", min: 1, max: 1)]
  ...
  @[Creme::SchemeFn("negative?", min: 1, max: 1)]
...
module Creme
  class Interpreter
    private def install_predicates(env : Env) : Array(String)
      register_module(Creme::R7RS::Predicates, env)
    end
  end
end
