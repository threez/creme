...
module Creme::R7RS::WriteLibrary
  extend self
  include Creme::BuiltinHelpers

  @[Creme::SchemeFn("display", min: 1, max: 2)]
  ...
  @[Creme::SchemeFn("write", min: 1, max: 2)]
  ...
  # faithful alias.
  @[Creme::SchemeFn("write-simple", min: 1, max: 2)]
  ...
  @[Creme::SchemeFn("write-shared", min: 1, max: 2)]
...
module Creme
  class Interpreter
    private def install_write(env : Env) : Array(String)
      register_module(Creme::R7RS::WriteLibrary, env)
    end
  end
end
