...
module Creme
  ...
  def self.to_scheme(v : Hash) : SchemeValue
    Creme.a_to_list(v.map { |k, val| Cons.new(SchemeStr.new(k.to_s), to_scheme(val)).as(SchemeValue) })
  end
  ...
  # Creme.list_to_a directly when a value is known to be list-shaped and an
