...
module Creme
  ...
  class VM
      ...
      loop do
        begin
          ...
          when Op::CaseMatch
            ...
            @stack.unsafe_put(base + instr.a, SchemeBool.of(datums.any? { |datum| Creme.scheme_eqv?(key, datum) }))
          ...
          when Op::ForLoopGuardedInc, Op::ForLoopGuardedDec
            ...
            still_current = Creme.scheme_eqv?(get_global_cached(frame, frame.ip - 1, instr.d), closure_of(frame))
            frame.ip += instr.b if in_range && still_current
          when Op::TestGlobalIdentity
            frame.ip += instr.b unless Creme.scheme_eqv?(get_global_cached(frame, frame.ip - 1, instr.a), closure_of(frame))
          when Op::Closure
    ...
    private def exec_prim(frame : CallFrame, base : Int32, instr : Instruction) : Nil
      ...
      when Op::Not
        regs[base + instr.a] = SchemeBool.of(!Creme.truthy?(regs[base + instr.b]))
      when Op::IsNull
      ...
      when Op::IsEq, Op::IsEqReturn
        regs[base + instr.a] = SchemeBool.of(Creme.scheme_eqv?(regs[base + instr.b], regs[base + instr.c]))
      when Op::IsEqImm
      ...
      when Op::IsEqUp
        x = regs[base + instr.b]
        y = upvalue(frame, instr.c).get
        regs[base + instr.a] = SchemeBool.of(Creme.scheme_eqv?(x, y))
      end
    end
      ...
      when QQList
        ...
        t.items.each do |item|
          if item.is_a?(QQSpliceItem)
            Creme.list_to_a(@stack.unsafe_fetch(hole_base + cur)).each { |x| items << x }
            cur += 1
          else
        ...
        {Creme.a_to_list(items, tail_value), cur}
      when QQVector
        items = [] of SchemeValue
        cur = idx
        t.items.each do |item|
    ...
    # Creme.record_type_names' order, out into the registers the
    ...
    private def exec_helper_form_local(frame : CallFrame, base : Int32, instr : Instruction) : Nil
      ...
      names = Creme.record_type_names(form).as(Array(String))
