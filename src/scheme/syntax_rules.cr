...
module Scheme
  ...
  class SchemeSyntaxRules < SchemeValue
    getter name : String
  ...
  private def self.match_into(pattern : SchemeValue, form : SchemeValue, literals : Array(String), bindings : Hash(String, SyntaxBinding)) : Bool
    ...
    when SchemeNil
      ...
      Scheme.scheme_equal?(pattern, form)
    end
  end
  ...
  private def self.match_ellipsis(sub_pattern : SchemeValue, rest_pattern : SchemeValue, form : SchemeValue, literals : Array(String), bindings : Hash(String, SyntaxBinding)) : Bool
    ...
    tail = cur
    min_rest = Scheme.proper_list?(rest_pattern) ? Scheme.list_to_a(rest_pattern).size : 0
    ...
    rest_form = Scheme.a_to_list(items[repeat_count..], tail)
  ...
  private def self.instantiate_list(template : Cons, bindings : Hash(String, SyntaxBinding)) : SchemeValue
    if (cdr = template.cdr).is_a?(Cons) && (second = cdr.car).is_a?(SchemeSym) && second.name == ELLIPSIS
      ...
      return Scheme.a_to_list(expanded, rest)
    end
  ...
  class Interpreter
    ...
    private def eval_define_syntax(expr : Cons, env : Env) : SchemeValue
    ...
    # returns its instantiated template, ready for the caller to set as
    # `expr` and trampoline through eval_core again.
    private def expand_syntax_rules(syntax : SchemeSyntaxRules, expr : SchemeValue) : SchemeValue
      syntax.rules.each do |pattern, template|
        ...
        if bindings = Scheme.match_pattern(pattern.cdr, expr.as(Cons).cdr, syntax.literals)
          return Scheme.instantiate_template(template, bindings)
        end
      end
