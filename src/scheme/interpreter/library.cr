...
# interpreter/base_library.cr. @global (the top-level program's own root
...
module Scheme
  class Interpreter
    ...
    # @base_env itself as a library (see interpreter/base_library.cr), and
    ...
    private def eval_define_library(expr : Cons, env : Env) : SchemeValue
    ...
    private def process_library_declarations(declarations : Array(SchemeValue), lib_env : Env, export_specs : Array(SchemeValue)) : Nil
      declarations.each do |decl|
        ...
        when "begin"
          args.each { |form| eval(form, lib_env) }
    ...
    # expression type (eval_include below, for eval_core to loop the
    # trampoline over — the same way it already does for begin's own
    # body). include-ci additionally case-folds the source text before
    ...
    private def include_file(filename : String, lib_env : Env, fold_case : Bool) : Nil
      read_include_file(filename, fold_case) { |forms| forms.each { |form| eval(form, lib_env) } }
    end

    # The standalone include/include-ci expression type (§4.1.7): reads and
    # concatenates every named file's forms into one array, for eval_core to
    # loop over — "the include or include-ci expression" is replaced by "a
    # begin expression containing what was read from the files," per
    # R7RS's own wording.
    ...
    private def eval_import(expr : Cons, env : Env) : SchemeValue
      Scheme.list_to_a(expr.cdr).each { |import_set| import_into(env, import_set) }
      NIL
    end

    private def import_into(into : Env, import_set : SchemeValue) : Nil
