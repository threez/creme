...
# src/scheme/eval/import.cr), exactly like the FFI-heavy libraries
# documented there already (tui/rfc8439/raft/jose/etc.).
require "./ffi_shim"

module Scheme::Builtins::FfiLibrary
  extend self
  include Scheme::BuiltinHelpers

  private KIND_NAMES = {
    "void"    => Scheme::CremeFfiShim::KIND_VOID,
    "int32"   => Scheme::CremeFfiShim::KIND_INT32,
    "int64"   => Scheme::CremeFfiShim::KIND_INT64,
    "double"  => Scheme::CremeFfiShim::KIND_DOUBLE,
    "bool"    => Scheme::CremeFfiShim::KIND_BOOL,
    "string"  => Scheme::CremeFfiShim::KIND_STRING,
    "pointer" => Scheme::CremeFfiShim::KIND_POINTER,
  }
  ...
  @[Scheme::SchemeFn("ffi-open", min: 1, max: 1)]
  ...
  def ffi_function(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    arg_kinds = Scheme.list_to_a(args[3]).map { |type_sym| kind_of(type_sym, "ffi-function") }

    prepared = Scheme::CremeFfiShim.prepare(handle, name.value, ret_kind, arg_kinds)
    SchemeBox.new("ffi-func", PreparedFn.new(prepared, ret_kind, arg_kinds), "#<ffi-func:#{name.value}>")
  end

  @[Scheme::SchemeFn("ffi-call", min: 2, max: 2)]
  def ffi_call(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    Scheme::CremeFfiShim.invoke(fn.prepared, arg_ptrs, ret_slot)
    marshal_return(fn.ret_kind, ret_slot)
  end

  @[Scheme::SchemeFn("ffi-pointer-ref", min: 3, max: 3)]
  def ffi_pointer_ref(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    raise SchemeRuntimeError.new("ffi-pointer-ref: type must not be void") if kind == Scheme::CremeFfiShim::KIND_VOID
    marshal_return(kind, (base + off.value).as(Void*))
  end

  @[Scheme::SchemeFn("ffi-pointer-set!", min: 4, max: 4)]
  def ffi_pointer_set(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    raise SchemeRuntimeError.new("ffi-pointer-set!: type must not be void") if kind == Scheme::CremeFfiShim::KIND_VOID
    marshal_arg_into(args[3], kind, (base + off.value).as(Void*), "ffi-pointer-set!")
    NIL
  end

  @[Scheme::SchemeFn("ffi-type-size", min: 1, max: 1)]
  def ffi_type_size(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    kind = kind_of(args[0], "ffi-type-size")
    size = case kind
           when Scheme::CremeFfiShim::KIND_INT32, Scheme::CremeFfiShim::KIND_BOOL
             4
           when Scheme::CremeFfiShim::KIND_INT64, Scheme::CremeFfiShim::KIND_DOUBLE,
                Scheme::CremeFfiShim::KIND_STRING, Scheme::CremeFfiShim::KIND_POINTER
             8
           else
  ...
  # buffer.
  @[Scheme::SchemeFn("ffi-gc-malloc", min: 1, max: 1)]
  ...
  @[Scheme::SchemeFn("ffi-gc-free", min: 1, max: 1)]
  ...
  @[Scheme::SchemeFn("ffi-pointer?", min: 1, max: 1)]
  ...
  @[Scheme::SchemeFn("ffi-null-pointer?", min: 1, max: 1)]
  ...
  private def marshal_arg_into(v : SchemeValue, kind : Int32, slot : Void*, who : String) : Nil
    ...
    when Scheme::CremeFfiShim::KIND_POINTER
      if v.is_a?(SchemeBool) && !v.value?
        slot.as(Void**).value = Pointer(Void).null
      else
  ...
  private def marshal_return(kind : Int32, slot : Void*) : SchemeValue
    case kind
    when Scheme::CremeFfiShim::KIND_VOID   then NIL
    when Scheme::CremeFfiShim::KIND_INT32  then SchemeInt.new(slot.as(Int32*).value.to_i64)
    when Scheme::CremeFfiShim::KIND_INT64  then SchemeInt.new(slot.as(Int64*).value)
    when Scheme::CremeFfiShim::KIND_DOUBLE then SchemeFloat.new(slot.as(Float64*).value)
    when Scheme::CremeFfiShim::KIND_BOOL   then SchemeBool.of(slot.as(Int32*).value != 0)
    when Scheme::CremeFfiShim::KIND_STRING
...
module Scheme
  class Interpreter
    register_library ["creme", "builtin", "ffi"], Scheme::Builtins::FfiLibrary
  end
end
