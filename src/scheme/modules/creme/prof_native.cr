...
# src/scheme.cr, so creme still builds on Alpine, just without
...
# (src/scheme/value/box.cr) — tagged "prof-report" rather than a dedicated
...
module Scheme::Builtins::ProfNative
  extend self
  ...
  @[Scheme::SchemeFn("profile", min: 1, max: 2)]
  ...
  @[Scheme::SchemeFn("profile-report?", min: 1, max: 1)]
  ...
  @[Scheme::SchemeFn("profile-total-samples", min: 1, max: 1)]
  ...
  @[Scheme::SchemeFn("profile-write-folded", min: 2, max: 2)]
  ...
  @[Scheme::SchemeFn("profile-write-speedscope", min: 2, max: 3)]
