...
module Scheme::Builtins::YamlLibrary
  extend self
  include Scheme::BuiltinHelpers

  @[Scheme::SchemeFn("yaml-read", min: 1, max: 1)]
  def yaml_read(interp : Interpreter, env : Env, args : Array(SchemeValue)) : SchemeValue
    ...
    Scheme.to_scheme(YAML.parse(s.value))
  rescue ex : YAML::ParseException
    raise SchemeRuntimeError.new("yaml-read: invalid yaml: #{ex.message}")
  end

  @[Scheme::SchemeFn("yaml-write", min: 1, max: 1)]
  ...
  private def yaml_write(v : SchemeValue, yaml : YAML::Builder, who : String) : Nil
    ...
    when Cons
      if yaml_alist?(v)
        ...
        yaml.sequence do
          Scheme.list_to_a(v).each { |e| yaml_write(e, yaml, who) }
        end
      end
    else
...
module Scheme
  class Interpreter
    register_library ["creme", "builtin", "yaml"], Scheme::Builtins::YamlLibrary
  end
end
